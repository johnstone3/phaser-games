...
export class DraggableGroup extends Phaser.GameObjects.Container {
    ...
    constructor(scene: Phaser.Scene, x: number, y: number, labels: string[]) {
        ...
        for (const label of labels) {
            ...
            const text: Phaser.GameObjects.Text = scene.add.text(boxWidth / 2, boxHeight / 2, label, { fontSize: '12px', fill: '#000' }).setOrigin(0.5);
            
            const boxContainer: Phaser.GameObjects.Container = scene.add.container(currentX, 0, [box, text]);
        ...
        hitZone.on('dragstart', (pointer: Phaser.Input.Pointer, draggedObject: Phaser.GameObjects.GameObject) => {
        ...
        hitZone.on('drag', (pointer: Phaser.Input.Pointer, draggedObject: Phaser.GameObjects.GameObject) => {
        ...
        hitZone.on('dragend', (pointer: Phaser.Input.Pointer, draggedObject: Phaser.GameObjects.GameObject) => {
            (this.scene as GameScene).updateGrid();
        });
